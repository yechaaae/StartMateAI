    ...
    async def polish_summary(
        ...
        *,
        ...
        prompt = (
            ...
            "- 분석 근거, 위험 요소, 전제(가정), 더 확인이 필요한 정보, 권장사항을 모두 자연스럽게 녹여 쓰세요.\n"
            "- 매우 중요: JSON의 내부 키 이름(예: missing_inputs, assumptions, evidence, ranking_basis, "
            "target, focused_section, recommendation 같은 영어 필드명)은 절대 그대로 쓰지 마세요. "
            "그 '값'만 일반 사용자가 바로 이해할 한국어 문장으로 풀어서 설명하세요. "
            "(예: missing_inputs → '아직 확인이 필요한 정보는 ~입니다', assumptions → '~라고 가정했어요')\n"
